...
//    private INotifier m_notifier;
    ...
    private IConnectCallback connCallback;

    Map<String, ConnectionDescriptor> m_clientIDs = new HashMap<String, ConnectionDescriptor>();

    private IAuthenticator m_authenticator = new IAuthenticator() {
        ...
        for (final Subscription sub : subscriptions.matches(topic)) {
            if (qos == QOSType.MOST_ONE) {
                ...
                if (!sub.isCleanSession() && !sub.isActive()) {
                    ...
                    //publish
                    notify(new NotifyEvent(sub.getClientId(), topic, qos, message, false, messageID));
                }
            }
        }
    }
